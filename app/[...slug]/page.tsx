...
export default async function DynamicPage({ params, searchParams }: PageProps) {
    ...
    const title = decodedSlug[depth - 1];

    // Check if data is an array (means we're at the notes level)
    if (Array.isArray(data)) {
        const notes = data as Note[];
        const sectionPath = `/${slug.join("/")}`;
        const unitSlug = slug.slice(0, -1).join("/");
    ...
    // Data is an object - list its keys
    const keys = Object.keys(data).filter(key => {
        const value = data[key];
        // Only show keys that have content (either arrays with items or objects with keys)
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {keys.map((key) => {
                    ...
                    return (
                        <Link
                            key={key}
                            href={`/${slug.join("/")}/${encodeURIComponent(key)}`}
                            className="p-5 rounded-lg border border-border hover:border-primary/50 transition-all card-hover group"
                            style={{ backgroundColor: 'var(--card)' }}
                        >
                            <div className="flex items-center gap-2 mb-2">
                                <FileText className="w-5 h-5 text-primary" />
                                <h3 className="text-lg font-semibold capitalize group-hover:text-primary transition-colors">{key}</h3>
                            </div>
                            <p className="text-muted-foreground text-sm">
                                {isArray ? `${itemCount} file${itemCount !== 1 ? 's' : ''}` : `${itemCount} item${itemCount !== 1 ? 's' : ''}`}
                            </p>
                        </Link>
                    );
                })}
                {keys.length === 0 && (
                    <p className="text-muted-foreground col-span-full text-center py-8">No content available.</p>
                )}
            </div>
